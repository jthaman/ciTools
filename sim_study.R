...
sim_study_ci <- function(n, fixed_ef, sigma_y, sigma_g, group_no, sims,
    ...
    for (i in 1:sims){
        ...
        tb <- add_ci(tb, fit, type = "sim", includeRanef = includeRanef, alpha = alpha, nSims = 1000, names = c("lwr_pi", "upr_pi"))
        tb <- add_ci(tb, fit, type = "boot", includeRanef = includeRanef, alpha = alpha, nSims = bootSims, names = c("lwr_boot", "upr_boot"))
