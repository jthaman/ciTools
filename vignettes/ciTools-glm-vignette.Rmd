...
```
>>>>>>> 7459716fcdf9795383f728c00eebedef033eadb8
...
$y$ is exactly $\{0,1\}$ in this Bernoulli-Logistic regression.
=======
not calculate prediction intervals for $y$ when $y$ is Bernoulli distributed
because the support of $y$ is exactly $\{0,1\}$. 
>>>>>>> 7459716fcdf9795383f728c00eebedef033eadb8
...
methods. In each simulation, a univariate GLM is fit on a variable
number of observations and a $95\%$ prediction interval was
calculated.
=======
parametric bootstrap. We focus on these intervals because we could 
not find results in the literature addressing their
performance. Our simulation is not comprehensive, so users of 
`ciTools` should exercise care when using  these methods. 
In each simulation, a simple $y = g^{-1}(mx + b)$ model is
