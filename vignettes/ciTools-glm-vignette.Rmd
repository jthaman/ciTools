...
```{r fig.width = 10, fig.heither = 7, fig.align = "center"}
...
```{r fig.width = 9, fig.heither = 7, fig.align = "center"}
...
performance, so users of `ciTools` should be more skeptical of these
methods. In each simulation, a simple $y = g^{-1}(mx + b)$ model is
fit on a variable number of observations and a $95\%$ prediction
...
interval width was also recorded, as were standard errosr of the
estimated coverage and mean interval width. Compared to a simulation
study of confidence intervals, we do not except interval widths to
shrink to $0$, due to the predictive error in the conditional response
distribution.

### Poisson

A simple Poisson model is fit with the log-link function. For each
sample size, $100$ simulations were performed. The total running time
of the simulation was about 7 minutes on an Intel i7 Broadwell
laptop. Calculated coverage probabilities generally appear to be close
to the nomial level. Average interval widths exhibit large variability.
...
dispersion parameter $\theta = 5$. For each sample size, $250$ models
were fit. The maximum sample size for this simulation was restricted
to $200$. Estimated coverage probabilities are generally higher than
the nominal level, but within simulation error according the standard
errors. Interval widths appear widely variable and exhibit no pattern
...
and simulated $100$ times. Gamma regression was not discussed in this
vignette but is still supported by `ciTools`. Estimated coverage
probabilities are generally close to the nomial level. Intervals widths
...
`ciTools`. Since a bootstrap is unnecessary, total running time for
the simulate is just a fraction of a minute. Coverage probabilities are
estimated within an standard error of $0.95$ and interval widths
generally decrease according to the increased sample size.
