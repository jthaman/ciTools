...
`ciTools` perfoms when one of the core functions (`add_ci`, `add_pi`,
...
bootstrap. The parametric method computes confidence intervals of the
...
type = "boot", ...)` and was included originally for making
comparisons against the parametric method. There are multiple methods
of bootstrap for regression models (resampling cases, resampling
residuals, parametric, etc.). The bootstrap method employed by
`ciTools` in `add_ci.glm()` resamples cases and iteratively refits the
model (using the default behavior of `boot::boot`) to determine
confidence intervals. After collecting the bootstrap replicates, a
bias-corrected and accelerated (BCa) bootstrap confidence interval is
formed.

There are several methods for computing bootstrap confidence
intervals, though we don't provide options to compute all of these
...
calculation methods. It's a fairly clear that the BCa intervals (red)
indeed exhibit little bias, but are not as tight as the parametric
intervals (purple). In practice, if the sample size is small or the
model is false, these interval estimates may exhibit more
disagreement.
...
only GLMs that we support with `add_pi()` are Guassian, Poisson,
...
probilities of prediction intervals formed using the parametric
...
that observed coverage levels are generally in line with the expected
level $95\%$. Interval widths generally decrease with sample size,
however there is an increase from sample size $500$ to $1000$, which
is within simulation error.
...
ggplot(pois, aes(x=sample_size, y=cov_prob)) + 
    ...
    ggtitle("Poisson Prediction Intervals", subtitle = "Coverage Probabilities on [0,1] scale") +
...
ggplot(pois, aes(x=sample_size, y=cov_prob)) + 
    ...
    xlab("Sample Size") +
    ylab("Coverage Probs +/- 2 SEs") +
    ggtitle("Poisson Prediction Intervals", subtitle = "Coverage Probabilities (zoom in)") +
    scale_x_log10(breaks = c(20, 30, 50, 100, 250, 500, 1000, 2000)) +
    geom_hline(aes(yintercept = 0.95), colour="#BB0000", linetype="dashed", size = 1)
```
...
ggplot(pois, aes(x=sample_size, y=int_width)) + 
    ...
    ggtitle("Poisson Prediction Intervals", subtitle = "Interval Widths") +
    geom_line(size = 1.5) +
    geom_point(size = 1.5)
```
...
probabilities are well below the nominal for small sample
...
ggplot(neg_bin, aes(x=sample_size, y=cov_prob)) + 
    ...
    xlab("Sample Size") +
    ylab("Coverage Probs +/- 2 SEs") +
    ggtitle("Negative Binomial Prediction Intervals", subtitle = "Coverage Probabilities on [0,1] scale") +
...
ggplot(neg_bin, aes(x=sample_size, y=cov_prob)) + 
    ...
    xlab("Sample Size") +
    ylab("Coverage Probs +/- 2 SEs") +
    ggtitle("Negative Binomial Prediction Intervals", subtitle = "Coverage Probabilities (zoom in)") +
    scale_x_log10(breaks = c(20, 30, 50, 100, 150, 200, 250, 500, 1000, 2000)) +
    geom_hline(aes(yintercept = 0.95), colour="#BB0000", linetype="dashed", size = 1)
```
...
ggplot(neg_bin, aes(x=sample_size, y=int_width)) + 
    ...
    xlab("Sample Size") +
    ylab("Interval Widths +/- 2 SEs") +
    ggtitle("Negative Binomial Prediction Intervals", subtitle = "Interval Widths") +
    geom_line(size = 1.5) +
    geom_point(size = 1.5)
```
...
close, though consistently slightly below the nomial level. Average
...
ggplot(gam, aes(x=sample_size, y=cov_prob)) + 
    ...
    xlab("Sample Size") +
    ylab("Coverage Probs +/- 2 SEs") +
    ggtitle("Gamma Regression Prediction Intervals", subtitle = "Coverage Probabilities on [0,1] scale") +
...
ggplot(gam, aes(x=sample_size, y=cov_prob)) + 
    ...
    xlab("Sample Size") +
    ylab("Coverage Probs +/- 2 SEs") +
    ggtitle("Gamma Regression Prediction Intervals", subtitle = "Coverage Probabilities (zoom in)") +
    scale_x_log10(breaks = c(100, 250, 500, 1000, 2000)) +
    geom_hline(aes(yintercept = 0.95), colour="#BB0000", linetype="dashed", size = 1)
```
...
ggplot(gam, aes(x=sample_size, y=int_width)) + 
    ...
    xlab("Sample Size") +
    ylab("Interval Widths +/- 2 SEs") +
    ggtitle("Gamma Regression Prediction Intervals", subtitle = "Interval Widths") +
    geom_line(size = 1.5) +
    geom_point(size = 1.5)
```

In practive, the (non-canonical) log-link function is more
...
ggplot(norm_log, aes(x=sample_size, y=cov_prob)) + 
    ...
    xlab("Sample Size") +
    ylab("Coverage Probs +/- 2 SEs") +
    ggtitle("Gaussian Regression Prediction Intervals", subtitle = "Coverage Probabilities on [0,1] scale") +
    scale_x_log10(breaks = c(20, 30, 50, 100, 250, 500, 1000, 2000)) +
    geom_hline(aes(yintercept = 0.95), colour="#BB0000", linetype="dashed", size = 1)
```
...
ggplot(norm_log, aes(x=sample_size, y=cov_prob)) + 
    ...
    xlab("Sample Size") +
    ylab("Coverage Probs +/- 2 SEs") +
    ggtitle("Gaussian Regression Prediction Intervals", subtitle = "Coverage Probabilities (zoom in)") +
    scale_x_log10(breaks = c(20, 30, 50, 100, 250, 500, 1000, 2000)) +
    geom_hline(aes(yintercept = 0.95), colour="#BB0000", linetype="dashed", size = 1)
```
...
ggplot(norm_log, aes(x=sample_size, y=int_width)) + 
    ...
    xlab("Sample Size") +
    ylab("Interval Widths +/- 2 SEs") +
    ggtitle("Gaussian Regression Prediction Intervals", subtitle = "Interval Widths") +
    geom_line(size = 1.5) +
    geom_point(size = 1.5)
```
