...
0)$. Logistic regression can be used to estimate $p|\texdff{x}$
where $\texdff{x}$ is a collection of predictor variables. This is
accomplished by assuming the relationship

$$log(\frac{p}{1-p}) = \texdff{x}\beta$$
...
its name. In $\texdff{R}$, this model can be fit using `glm` by
