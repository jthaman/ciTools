...
1 - no missing data in the design matrix.
...
The purpose of this vignette is to introduce and discuss some of
the new capabilities that have been baked into `ciTools` to handle
accelerated failure time (AFT) models. Some of the new AFT methods
in `ciTools` are more informative than methods for
previous models, and will inform future development decisions that
will be made in `ciTools`. In particular, `ciTools` now supports
intervals for estimated survival time probabilities and quantiles
for a range of common AFTs.
...
where $\varepsilon \sim F$ to make the linear effect of $X\beta$
...
making predictions. `predict.survreg` for example, allows one to make
predictions with the median of the conditional AFT model
(`predict.survreg(fit, newdata, method = "quantile", p = 0.5)`), or
with the just the linear predictor $\exp(X\beta)$
(`predict.survreg(fit, newdata)`).  The former is a bona fide
estimator of the population median, while the latter (generally) is
not an estimator of a population mean or median.
...
Here are some of the mathematical ideas that go into the above
...
path <- "~/git/ciTools/vignettes/survreg_data"
...
$1-\alpha/2$ quantiles of the estimated conditional distribution. This
method is simple and works reasonably well in the absence of
censoring, as displayed in the plots below.
...
account for this uncertainty in the model coefficients. This is
essentially what we have done to produce prediction intervals for GLMs
as well, just applied to the new class of AFT models.
...
<!-- ```{r, include = FALSE, cache= TRUE} -->
<!-- expq0 <- read_csv(paste0(path,"/","exponential_sim_quantile_ci_censoredp00.csv")) -->
<!-- expq1 <- read_csv(paste0(path,"/","exponential_sim_quantile_ci_censoredp03.csv")) -->
<!-- expq2 <- read_csv(paste0(path,"/","exponential_sim_quantile_ci_censoredp05.csv")) -->
<!-- expq_dat <- rbind(expq0, expq1, expq2) %>% -->
<!--     mutate(distr = "Exponential") -->
<!-- loglogq0 <- read_csv(paste0(path,"/", -->
<!--                             "loglogistic_sim_quantile_ci_scale025_censoredp00.csv")) -->
<!-- loglogq1 <- read_csv(paste0(path,"/", -->
<!--                             "loglogistic_sim_quantile_ci_scale025_censoredp03.csv")) -->
<!-- loglogq2 <- read_csv(paste0(path,"/", -->
<!--                             "loglogistic_sim_quantile_ci_scale025_censoredp05.csv")) -->
<!-- loglogq_dat <- rbind(loglogq0, loglogq1, loglogq2) %>% -->
<!--     mutate(distr = "Loglogisic") %>% -->
<!--     dplyr::select(-c(scale)) -->
<!-- lognormq0 <- read_csv(paste0(path,"/","lognormal_sim_quantile_ci_scale2_censoredp00.csv")) -->
<!-- lognormq1 <- read_csv(paste0(path,"/","lognormal_sim_quantile_ci_scale2_censoredp03.csv")) -->
<!-- lognormq2 <- read_csv(paste0(path,"/","lognormal_sim_quantile_ci_scale2_censoredp05.csv")) -->
<!-- lognormq_dat <- rbind(lognormq0, lognormq1, lognormq2) %>% -->
<!--     mutate(distr = "Lognormal") %>% -->
<!--     dplyr::select(-c(scale)) -->
<!-- weibullq0 <- read_csv(paste0(path,"/","weibull_sim_quantile_ci_scale2_censoredp00.csv")) -->
<!-- weibullq1 <- read_csv(paste0(path,"/","weibull_sim_quantile_ci_scale2_censoredp03.csv")) -->
<!-- weibullq2 <- read_csv(paste0(path,"/","weibull_sim_quantile_ci_scale2_censoredp05.csv")) -->
