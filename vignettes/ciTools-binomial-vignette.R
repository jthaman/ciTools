...
suppressWarnings(suppressMessages(library(ciTools)))
...
#' `tb` is a single success out of 5 attempts with a covariate of `r
#' round(tb$x[1], 3)`.  Another way to think of this is five Bernoulli
...
#' `add_quantile`, and `add_probs`) produce different behaviors, in
#' large part because prediction intervals, quantiles, and probability
#' values are not very useful for Bernoulli variables.
#' 
#' For example, consider prediction intervals. These are used to
#' quantify the observation-to-observation variability and define its
...
#' multiple levels, making it sensible to consider prediction
#' intervals, quantiles, or probabilities. For example, one may wish
#' to consider the value at which we can expect to observe 90 percent
#' of responses:
