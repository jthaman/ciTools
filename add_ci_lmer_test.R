...
Ran_True <- add_ci.lmerMod(tb, mm, condition_RE = TRUE)
Ran_False <- add_ci.lmerMod(tb, mm, condition_RE = FALSE)
...
ggplot(aes(x=x, y=pred, ymin=LCB0.025, ymax=UCB0.975, color=Predict.Method), data=comp.data[c(1:30,
                                                                                    181:210,
                                                                                    181:210+180
                                                                                    ),]) +
  geom_point() + 
